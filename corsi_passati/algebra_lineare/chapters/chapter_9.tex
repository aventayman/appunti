...
\pf{Dimostrazione}{Sia \(a\) un asse della conica. Poniamo i suoi parametri direttori di \(a\) come \(p.d.a = [(l,m)]\). Ma allora \(D\) cioè il polo di \(a\) ha coordinate affini \([(-m,l,0)]\), poiché risiede sulla retta di direzione ortogonale all'asse. Quindi possiamo impostare il seguente prodotto fra matrici \[
\left( \; \begin{matrix}
    -m & l & 0 \\
\end{matrix} \; \right) A
\left( \; \begin{matrix}
