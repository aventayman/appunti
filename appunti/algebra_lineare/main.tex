...
\thm{}{Sia \(S = (v_{1}, v_{2}, \ldots, v_n)\) una sequenza di vettori di \(V_n(K)\) con, \(p \le n\), dello spazio vettoriale \(V_n(K)\) è libera se, e soltanto se, dalla matrice \(M\), che ha nelle proprie righe (o colonne) le componenti dei vettori di \(M\) in una base di \(V_n(K)\), si può estrarre un minore di ordine \(p\) non singolare (cioè con determinante non nullo).}
