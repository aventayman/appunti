...
\mprop{}{Siano \(\alpha: ax + by + cz + d = 0\) e \(\beta: a'x + b'y + c'z + d' = 0\) due piani in \(E_3(\RR)\), allora \(\alpha \perp \beta \iff a a' + b b' + c c' = 0\) }
...
\mprop{Ortogonalità tra retta e piano}{Siano \(r:\) con \(P.d.r = [(l,m,n)]\) e sia \(\alpha\) di equazione \(ax + by + cz + d= 0\) una retta e un piano di \(E_3(\RR)\). Allora \(r \perp \alpha\) se e soltanto se \([(a,b,c)] = [(l,m,n)]\) }
...
\dfn{Distanza tra 2 punti in \(E_{3}(\RR)\) }{Siano \(P = (x_{1} , x_{2}, ..., x_{n} )\) e \(Q = (x_{1} ' x_2 ', ..., x_n ')\). La distanza tra \(P\) e \(Q\) è la norma del vettore \(\vec{PQ}\) \[
...
\] }
Caso \(E_{3}(\RR)\) da aggiungere

\dfn{Distanza tra punto e retta}{Siano \(P = (x_0, y_0)\) e \(r = [Q, V_1]\) rispettivamente un punto e una retta in \(E_{2}(\RR)\). Definiamo la \textbf{distanza tra il punto \(P\) e la retta \(r\)} come la distanza tra \(P\) e il punto \(H\), piede della perpendicolare per \(P\) a \(r\) (cioè l'intersezione tra \(r\) e la retta perpendicolare a \(r\) passante per \(P\)).}
...
\dfn{Distanza punto piano}{Siano \(P = (x_0, y_0, x_0)\) e \(\alpha : ax + by + cz + d = 0\) un punto e un piano di \(E_{3}(\RR)\). Definiamo \textbf{la distanza} \(d(P, \alpha)\) come la distanza tra \(P\) e il punto \(H\) intersezione tra \(\alpha\) e la retta per \(p \perp \alpha \).}
...
\dfn{Distanza tra un punto e una retta in \(E_{3}(\RR)\) }{Siano \(P\) e \(r = [Q, V_1]\) un punto e una retta in \(E_{3}(\RR)\). Sia \(\alpha \) il piano per \(P\) ortogonale a \(r\) e sia \(H\) l'intersezione tra \(r\) e \(\alpha \). Definiamo \(d(P, r) = d(P, H) = ||\vec{{PH}}||\).}
...
\dfn{Retta di minima distanza}{Si dice \textbf{retta di minima distanza} tra due rette \(r, s\) sghembe in \(E_{3}(\RR)\) una retta ortogonale e incidente sia a \(r\) che a \(s\).}
...
\dfn{Assi}{In \(E_{2}(\RR)\) dati due punti \(P,Q\), si dice \textbf{asse} del segmento \(\ol{PQ}\) la retta passante per il punto medio di \(P\) e \(Q\) e ortogonale al segmento \(\ol{PQ}\).  }
...
\dfn{Piano assiale}{In \(E_{3}(\RR)\) si dice \textbf{piano assiale} del segmento \(\ol{PQ}\) il piano \(\alpha \) passante per il punto medio di \(P\) e \(Q\) e ortogonale al segmento \(\ol{PQ}\).}

\mprop{}{Il piano assiale del segmento \(\ol{PQ}\) è il luogo dei punti equidistanti tra \(P\) e \(Q\).}

\section{Circonferenze in \(E_{2}(\RR)\) }
\dfn{Circonferenza}{Dato un punto \(C = (x_0, y_0)\) in \(E_{2}(\RR)\) e dato \(r\) numero reale positivoSi dice circonferenza di centro \(C\) e raggio \(r\) il luogo dei punti aventi distanza \(r\) da \(C\). }
...
\mprop{}{Siano \(A, B, C, D\) quattro punti non complanari di \(E_{3}(\RR)\). Per \(A, B, C, D\) passa un'unica sfera}
...
\section{Circonferenze in \(E_{3}(\RR)\) }
\dfn{Circonferenza in \(E_{3}(\RR)\) }{Dati un piano \(\alpha \), un suo punto \(C\) e un numero reale positivo \(r\). Si dice \textbf{circonferenza} di raggio \(C\) e raggio \(r\) il luogo dei punti di \(\alpha \) aventi distanza \(r\) da \(C\).}
...
\] Ed è una curva algebrica reale di \(E_{3}(\RR)\).}
